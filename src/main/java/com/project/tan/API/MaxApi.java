 ...
 * <p>
 * 注意：事实上，对于 Aggregations 函数，Flink 帮助我们封装了状态数据，这些状态数据不会被清理，所以在实际生产环境中应该尽量避免在一个无限流上使用 Aggregations。
...
//        items.keyBy(0).maxBy(2).printToErr();
        ...
        SingleOutputStreamOperator<Tuple3<Integer, Integer, Integer>> reduce = itemsReduce.keyBy(0).reduce(new ReduceFunction<Tuple3<Integer, Integer, Integer>>() {
            @Override
